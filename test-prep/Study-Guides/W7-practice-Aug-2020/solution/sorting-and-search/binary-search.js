...
function binarySearchIndex(array, target, lo = 0, hi = array.length - 1) {
	// I'm using a > here instead of ===. We should account for the possibility
	// that we are looking for the last element. If we used === we would first have
	// to include a base case to see if our element at that location was our target
	// (An edge case that I don't think Alvin accounts for)
	if (lo > hi) {
		return -1;
	}
